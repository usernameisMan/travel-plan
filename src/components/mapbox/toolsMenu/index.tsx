...
const ToolsMenu: React.FC<Props> = ({ className, onClickMenu = (fileName: string) => { }, onClose }) => {
  ...
  return (
    <div className={cn(
      ...
      <div className={cn(
        "p-3",
        isMobile ? "grid grid-cols-3 gap-2" : "grid grid-cols-2 gap-2"
      )}>
        {Array.isArray(markers) && markers.map((marker) => (
          ...
          >
            <div className="w-12 h-12 rounded-xl bg-white border-2 border-purple-200 flex items-center justify-center shadow-sm">
              <Image 
