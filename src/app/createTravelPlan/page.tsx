...
const TravelPlan = () => {
  ...
  const onAddOneMarker = useCallback((fileName: string, lng: string, lat: string) => {
    setTracks((prev: any) => [...prev, {
      type: fileName, lng, lat
    }])
  }, []);

  return (
    <div className={cn('w-full h-full flex')}>
      <CreateMarkerDialog onOpenChange={onOpenChange} open={createMarkerDialogIsOpen} setOpenCreateMarkerDialog={setOpenCreateMarkerDialog} />
